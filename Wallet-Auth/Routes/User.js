...
app.post("/addUser",(req,res)=>{
    ...
    if(!user.name || !user.email || !user.number || !user.password || !user.fcmToken){
        res.status(200).send([{message:'error'}])
        return
    }

    Otp.findOne({number:user.number}).exec().then((otpDoc)=>{
...
app.post("/getUsersWithContacts",(req,res)=>{
    ...
    Users.find({},["name","number","email"]).where("number").in(contacts).exec()
         .then((doc)=>{
             res.json(doc)
         }).catch((err)=>{
             res.send(err)
...
app.get("/getUsers",(req,res)=>{
     Users.find({},["name","number","email"]).exec().then((doc)=>{
