...
func sendNotification(devices []string, fromName string,from string, amount string,notifyType string,fromEmail string,) {
	...
	c.NewFcmRegIdsMsg(devices, map[string]string{
		"type": notifyType+"," + fromName + "," + from + "," + amount+","+fromEmail,
	})
	c.Send()
}
