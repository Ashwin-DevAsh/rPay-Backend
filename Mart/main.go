...
func handelRequest() {
	r.HandleFunc("/order", func(response http.ResponseWriter, request *http.Request) {
		if request.Method == "POST" {
			...
			var orderDatabase OrderDatabase;
			if doOrder(db, orderData, &transactionID, &transactionTime,&orderDatabase) {
				...
				if err != nil {
					...
					notify(orderData.TransactionData.From.Id,  orderData.TransactionData.From.Id , orderData.TransactionData.To.Name, orderData.TransactionData.Amount,"rmartPayment",orderData.TransactionData.To.Email)
